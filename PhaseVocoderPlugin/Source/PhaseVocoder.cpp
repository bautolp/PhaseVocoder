...
PhaseVocoder::PhaseVocoder(uint32_t window_size, uint32_t hop_size)
    : m_window_size(window_size), m_hop_size(hop_size), m_forward_fft(NULL), m_reverse_fft(NULL), m_window_bufer(NULL)
{
...
{
    for (float *buff_pos = 0; buff_pos < (buff_size - m_window_size); buff_pos++)
    {
